...
export class Memory {
    ...
    public setLatestSavedItems(items: Item[]) {
        localStorage['latestitems'] = JSON.stringify(items);
    }

    public getLatestSavedItems(): Item[] {
        try {
            return JSON.parse(localStorage['latestitems']);
        } catch (e) {
            return [];
        }
    }
